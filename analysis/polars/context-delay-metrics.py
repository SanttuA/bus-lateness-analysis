from __future__ import annotations

import argparse

from _shared import (
    add_bucket_arg,
    add_cache_args,
    add_common_args,
    add_quality_args,
    add_timezone_arg,
    print_or_empty,
    write_optional_csv,
)
from cli_common import load_delay_buckets_for_args
from report_cache import build_context_delay_metrics


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Report robust delay metrics by line, direction, local hour, and day type."
    )
    add_common_args(parser)
    add_timezone_arg(parser)
    add_quality_args(parser)
    add_bucket_arg(parser)
    add_cache_args(parser)
    parser.add_argument("--line-ref")
    parser.add_argument("--direction-ref")
    parser.add_argument("--day-type", choices=("all", "weekday", "weekend"), default="all")
    parser.set_defaults(limit=50, min_observations=30)
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    result = build_context_delay_metrics(
        load_delay_buckets_for_args(args),
        line_ref=args.line_ref,
        direction_ref=args.direction_ref,
        day_type=args.day_type,
        min_observations=args.min_observations,
        limit=args.limit,
    )
    print_or_empty(result)
    write_optional_csv(result, args.output_csv)


if __name__ == "__main__":
    main()

